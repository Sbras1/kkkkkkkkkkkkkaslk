...
bot.onText(/^\/web$/i, async (msg) => {
    ...
    // حفظ التوكن مع الآيدي + وقت الانتهاء (24 ساعة)
    traderTokens.set(token, {
        userId: userId,
        expiresAt: Date.now() + 86400000 // 24 ساعة
    });
...
app.get('/trader', async (req, res) => {
    ...
    if (!tokenData) {
        return res.status(403).send(`
            <div style='text-align:center; font-family:Tajawal,sans-serif; margin-top:50px; padding:20px'>
                ...
                <p style='color:#777'>السبب: التوكن غير موجود (ربما تم إعادة تشغيل السيرفر)</p>
    ...
    if (Date.now() > tokenData.expiresAt) {
        const hoursAgo = Math.floor((Date.now() - tokenData.expiresAt) / 3600000);
        return res.status(403).send(`
            <div style='text-align:center; font-family:Tajawal,sans-serif; margin-top:50px; padding:20px'>
                <h2>⛔ الرابط منتهي الصلاحية</h2>
                <p style='color:#777'>انتهى منذ: ${hoursAgo} ساعة</p>
                <p style='color:#777'>اكتب <code>/web</code> في البوت للحصول على رابط جديد.</p>
            </div>
        `);
    }
