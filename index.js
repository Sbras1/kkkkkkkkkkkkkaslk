...
bot.onText(/^\/start/i, async (msg) => {
  ...
  // رسالة ترحيب للجميع (مشتركين وغير مشتركين)
  let welcome = "";
  
  if (!isTrader(userId)) {
    welcome = "مرحباً بك! 👋\n\n";
    welcome += "🎁 **ميزة مجانية:**\n";
    welcome += "• 🎮 استعلام عن ID اللاعب\n\n";
    welcome += "💎 **للحصول على جميع الميزات (اشتراك مدفوع):**\n";
    welcome += "• 🧪 فحص الأكواد\n";
    welcome += "• ⚡ تفعيل الأكواد (فردي وجماعي)\n";
    welcome += "• 📒 السجلات والدعم الفني\n\n";
    welcome += "📞 للاشتراك: @Sbras_1\n\n";
    welcome += "استخدم القائمة أدناه 👇";
  } else {
    welcome = "أهلاً بك في بوت تاجر PUBG 💳\n\n";
    welcome += formatTraderAccount(msg.from, info);
    welcome += "\n\nمميزات البوت اختر زر الاشتراك بالأسفل.:\n";
    welcome += "اختر الخدمة التي تريد استخدامها من القائمة أدناه.";
  }
...
bot.on("message", async (msg) => {
  ...
  if (text === "💳 الاشتراك") {
      const demoInlineKeyboard = {
          reply_markup: {
              inline_keyboard: [
                  // الصف الأول: تفعيل كود - تفعيل جماعي
                  [
                      { text: "⚡ تفعيل كود", callback_data: "demo_charge" },
                      { text: "🚀 تفعيل جماعي", callback_data: "demo_clan" }
                  ],
                  // الصف الثاني: تنشيط حسابي (زر واحد في المنتصف)
                  [
                      { text: "🔓 تنشيط حسابي", callback_data: "demo_activate" }
                  ],
                  // الصف الثالث: استعلام - فحص
                  [
                      { text: "🎮 استعلام عن ID", callback_data: "demo_player" },
                      { text: "🧪 فحص كود", callback_data: "demo_check" }
                  ]
              ]
          }
      };
      ...
      await bot.sendMessage(chatId, 
          ...
          "💰 السعر: 55 ريال / 15$ - شهر\n\n" +
          "👇 اضغط على الأزرار أدناه لتكتشف مميزات البوت:", 
          demoInlineKeyboard
      );
      return;
  }
  ...
  // غير تاجر؟ نرجع رسالة منع (إلا إذا ضغط زر التذكرة أو في وضع استعلام مجاني)
  if (!isTrader(userId) && text !== "🎫 فتح تذكرة" && session.mode !== "WAIT_PLAYER_LOOKUP_ID") {
    const txt =
      "⚠️ هذا البوت مخصص لتجّار PUBG فقط.\n\n" +
      "لا يمكنك استخدام هذه الميزة قبل الاشتراك كتاجر.\n\n" +
      "💡 الميزة المجانية: استعلام عن ID\n\n" +
      "للاشتراك أو الاستفسار:\n" +
      "• راسل مالك البوت على تيليجرام: @Sbras_1";
    await bot.sendMessage(chatId, txt);
    return;
  ...
  if (session.mode === "WAIT_BULK_IDS") {
    ...
    if (ids.length < 2 || ids.length > 5) {
      return bot.sendMessage(chatId, "❌ العدد او الايدي غير صحيح!\nيجب إرسال من 2 إلى 5 آيديات في الرسالة الواحدة.\nحاول مرة أخرى.");
    }
...
bot.on("callback_query", async (query) => {
  ...
  // ============================================================
  // 💎 معالجات أزرار شرح المميزات (Demo Callbacks)
  // ============================================================

  // 1. شرح: تفعيل كود (فردي + ستاك)
  if (data === "demo_charge") {
      const caption = 
          "⚡ شرح ميزة: تفعيل كود (نظامين في زر)\n\n" +
          "عند تفعيل هذا الخيار، يطلب البوت الآيدي أولاً للتحقق، ثم يخيرك:\n\n" +
          "1️⃣ كود واحد (Single): للسرعة، مع تحليل ذكي لرد الموقع.\n" +
          "2️⃣ مجموعة أكواد (Stack): اشحن حتى 5 أكواد لنفس اللاعب دفعة واحدة بتقرير مفصل.\n\n" +
  ...
  // 2. شرح: تفعيل جماعي (كلان)
  if (data === "demo_clan") {
      const caption = 
          "🚀 شرح ميزة: شحن جماعي\n\n" +
          "اشحن لعدة لاعبين في وقت قياسي!\n\n" +
          "📝 الطريقة:\n" +
          "1. أرسل قائمة الآيديات (2-5 لاعبين).\n" +
          "2. البوت يفحص الأسماء.\n" +
          "3. أرسل قائمة الأكواد.\n" +
          "4. يتم الشحن بالتتابع تلقائياً.";
  ...
  if (data === "demo_player") {
      await bot.sendMessage(chatId, 
          ...
          "فقط أرسل الآيدي (مثال: 5555555) وسيحضر لك:\n" +
          "• اسم اللاعب.\n" +
          "• الآيدي (منسوخ).\n\n" +
          "💾 جديد: زر لحفظ اللاعب في جهات الاتصال."
      );
      await bot.answerCallbackQuery(query.id);
      return;
  }
  ...
  if (data === "demo_check") {
      await bot.sendMessage(chatId, 
          ...
          "🟢 جديد: سليم وغير مستخدم.\n" +
          "🔴 مستخدم: يظهر لك وقت الاستخدام بالضبط.\n" +
          "❌ تالف: الكود غير صحيح."
      );
      await bot.answerCallbackQuery(query.id);
      return;
  ...
  // 5. شرح: تنشيط الحساب
  if (data === "demo_activate") {
      await bot.sendMessage(chatId, 
          "🔓 تفعيل الاشتراك الآلي\n\n" +
          "1️⃣ اشترِ \"كود تفعيل\" \n" +
          "2️⃣ اكتب الأمر: /تفعيل KEY-Q6JSxxxx\n" +
          "3️⃣ يعمل البوت فوراً.\n\n" +
          "🛒 للشراء: تواصل مع @Sbras_1"
      );
      await bot.answerCallbackQuery(query.id);
      return;
  }
...
// ==================================================
// 🎫 الرد من الإدارة إلى التاجر (نظام ذكي مع دعم الخصوصية)
// ==================================================

// الرد من الإدارة (من داخل القروب) إلى التاجر - نظام ذكي
bot.on("message", async (msg) => {
    // التأكد أن الرسالة قادمة من قروب الإدارة وأنها "رد" (Reply)
    if (msg.chat.id === ADMIN_GROUP_ID && msg.reply_to_message && msg.text && !msg.text.startsWith('/')) {
        
        let customerId = null;
        let foundMethod = null; // لتتبع الطريقة الناجحة

        // المحاولة 1: جلب الآيدي من التوجيه (إذا كان حسابه مفتوح)
        if (msg.reply_to_message.forward_from) {
            customerId = msg.reply_to_message.forward_from.id;
            foundMethod = "forward";
        } 
        
        // المحاولة 2: جلب الآيدي من نص الرسالة (البطاقة)
        // نبحث عن أي رقم بعد "🆔 الآيدي:" سواء مع أو بدون backticks
        if (!customerId && msg.reply_to_message.text) {
            // نمط مرن يقبل: 🆔 الآيدي: 123456789 أو 🆔 الآيدي: `123456789`
            const match = msg.reply_to_message.text.match(/🆔 الآيدي:?\s*`?(\d+)`?/);
            if (match) {
                customerId = match[1];
                foundMethod = "card";
            }
        ...
        // إذا وجدنا الآيدي، نرسل الرد
        if (customerId) {
            try {
                await bot.sendMessage(customerId, `👨‍💻 رد من الدعم الفني:\n\n${msg.text}`);
                await bot.sendMessage(ADMIN_GROUP_ID, `✅ تم إيصال الرد (${foundMethod === 'card' ? 'عبر البطاقة' : 'مباشر'})`, { reply_to_message_id: msg.message_id });
            } catch (err) {
                await bot.sendMessage(ADMIN_GROUP_ID, "❌ فشل الإرسال (ربما حظر البوت)", { reply_to_message_id: msg.message_id });
            }
        } 
        // فقط إذا كانت رسالة محولة لكن بدون بيانات المرسل → إرشاد
        else if (msg.reply_to_message.forward_date) {
            await bot.sendMessage(ADMIN_GROUP_ID, 
                "💡 للرد على هذا الشخص:\n" +
                "ابحث عن رسالة البطاقة (التي تبدأ بـ 🎫 تذكرة جديدة) وقم بالرد عليها بدلاً من الرسالة المحولة.",
